...
/// Environment override for [`reduces_wide_last_axis_on_cuda`].
///
/// `1` asserts the loaded runtime reduces a wide last axis efficiently, `0`
/// asserts it does not. Unset defers to the version check.
const WIDE_ARG_REDUCTION_VAR: &str = "ONNX_GENAI_DIRECT_WIDE_ARGMAX";
...
/// while the fix exists only as an unmerged pull request. Nothing in this
...
pub fn reduces_wide_last_axis_on_cuda() -> bool {
    ...
    *ANSWER.get_or_init(|| {
        match std::env::var(WIDE_ARG_REDUCTION_VAR).as_deref() {
            Ok("1") => return true,
            Ok("0") => return false,
            _ => {}
        }
