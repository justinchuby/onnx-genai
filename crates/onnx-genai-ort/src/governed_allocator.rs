...
impl GovernedAllocator {
    ...
    /// physically on demand. `GovernedAllocator` reports whether the installed
    /// allocator advertises the [`VirtualBacking`] capability, so a session
    /// that registers one of these answers the same question the native path
    /// does -- which is what lets a consumer size a KV cache without knowing
