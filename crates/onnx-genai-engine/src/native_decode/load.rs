...
impl NativeDecodeSession {
    ...
    pub(crate) fn load_with_weight_offload_host_cache(
        ...
        csa::refuse_compressed_records_on_cuda(
            options
                ...
                .unwrap_or_default(),
            matches!(&device, NativeDecodeDevice::Cuda { .. }) && !cfg!(feature = "native-cuda"),
        )?;
    ...
    pub(crate) fn from_session_with_cuda_options_and_io(
        ...
        let compressed_state_stats = if cuda.is_some() {
            let device_allocations =
                u64::try_from(compressed_state.device_binding_count()).unwrap_or(u64::MAX);
            CompressedStatePathStats {
                device_allocations,
                telemetry_updates: u64::from(device_allocations > 0),
