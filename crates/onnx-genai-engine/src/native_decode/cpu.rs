    ...
    pub(crate) fn decode_cpu(
        ...
        for (metadata, tensor) in self.session.outputs().iter().zip(outputs) {
            if metadata.name == self.logits {
                ...
                // not the token count, and the ABI declares this by role because
                // it is not inferable from the tensor shape alone.
                let csa_record = self.csa_record_presents.contains(&metadata.name);
