...
impl Engine {
    ...
    fn create_native_session_state(&mut self) -> anyhow::Result<SessionId> {
        ...
        let id = self.bump_native_access();
        let access = self.bump_native_access();
        ...
        self.evict_native_sessions(None)?;
        Ok(id)
    }

    #[cfg(feature = "native-backend")]
    fn evict_native_sessions(&mut self, protect: Option<SessionId>) -> anyhow::Result<()> {
        loop {
            ...
            let victim = self
                ...
                .context("native session eviction requested with no sessions")?;
    ...
    pub fn rewind_session_to(
        ...
        if self.decode_backend == EngineDecodeBackend::Native {
            ...
            if self.native_active_session == Some(session_id) {
                ...
                state.estimated_kv_bytes = native.estimated_kv_bytes();
            } else {
                state.estimated_kv_bytes = 0;
            }
            self.evict_native_sessions(Some(session_id))?;
            return Ok(());
        }
        self.require_ort_backend("session rewind")?;
        let position = position.get();
...
impl Engine {
    ...
    fn generate_native_in_session_impl(
        ...
        let (result, estimated_kv_bytes) = {
            ...
            let estimated_kv_bytes = native.estimated_kv_bytes();
            (result, estimated_kv_bytes)
        };
