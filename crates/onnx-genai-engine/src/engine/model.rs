...
pub struct Engine {
    ...
    /// Byte budget above which inactive native sessions are dropped.
    #[cfg(feature = "native-backend")]
...
pub(crate) struct NativeSessionState {
    ...
    /// Estimated KV bytes this session currently holds. Only the active session
    /// owns KV memory, so this is zero for every other session.
