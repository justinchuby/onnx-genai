...
mod tests {
    ...
    fn degenerate_sampler_argmax_is_tiled_into_two_stages() {
        ...
        assert!(
            graph.node.iter().all(|node| node.domain.is_empty()),
            "portable lowering must contain only standard ONNX nodes"
        );
    ...
    fn first_argmax(values: &[f32]) -> usize {
        values
            .iter()
            .enumerate()
            .fold((f32::NEG_INFINITY, 0), |(best, index), (at, &value)| {
                if value > best {
                    (value, at)
                } else {
                    (best, index)
    ...
    fn tiled_first_argmax(values: &[f32], rows: usize, tile: usize) -> usize {
        assert_eq!(values.len(), rows * tile);
        let extrema = values
            .chunks_exact(tile)
            .map(|chunk| chunk[first_argmax(chunk)])
            .collect::<Vec<_>>();
        let outer = first_argmax(&extrema);
        outer * tile + first_argmax(&values[outer * tile..(outer + 1) * tile])
    }

    #[test]
    fn tiled_semantics_match_direct_for_wide_rows_and_ties() {
        let (rows, tile) = plan_tiles(202_048).expect("wide vocabulary tiles");
        let mut cases = vec![
            vec![2.0; 202_048],
            vec![f32::NEG_INFINITY; 202_048],
            (0..202_048)
        ...
        cases[1][202_047] = -1.0;
        cases[2][5] = 2_000.0;
        cases[2][202_043] = 2_000.0;
        for values in cases {
            assert_eq!(
                tiled_first_argmax(&values, rows as usize, tile as usize),
                first_argmax(&values)
            );
        }
    }
