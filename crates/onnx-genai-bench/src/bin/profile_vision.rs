//! Native nxrt versus ONNX Runtime CPU inference profiler for vision models.
...
#[derive(Debug, Parser)]
#[command(about = "Compare native nxrt and ONNX Runtime CPU inference on a vision model")]
struct Args {
    /// ONNX model file.
    #[arg(long)]
    model: PathBuf,
            ...
            let (native, ort) = match (input.dtype, ort_input.dtype) {
                ...
                (native, ort) => bail!(
                    "input '{}' has unsupported or mismatched dtype: native={native:?} ORT={ort:?}; \
                     profile_vision currently synthesizes Float32 and Int64 inputs",
                    input.name
                ),
            };
            Ok(InputPair {
    ...
    let inputs = session
        ...
        .map(|(index, input)| {
            ...
            let value = match input.dtype {
                ...
                dtype => bail!(
                    "input '{}' has unsupported dtype {dtype:?}; profile_vision currently \
