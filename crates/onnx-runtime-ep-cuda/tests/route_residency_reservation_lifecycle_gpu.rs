...
use onnx_runtime_ep_cuda::{CudaExecutionProvider, RoutePrepareCommitInterlock};
...
fn finalize_artifacts(
    ...
    provider
        ...
        .map(onnx_runtime_ep_api::ExecutorArtifactReport::into_state)
}
...
fn real_producer_installs_executor_scoped_banks_once() {
    ...
    let first = artifact_scope(&provider);
    let second = artifact_scope(&provider);
    ...
    for scope in [first, second] {
        let status = provider.route_residency_executor_status(scope.executor);
    ...
    let first_ranges: Vec<_> = bank
        ...
        .map(|&value| {
            provider
                .residency()
                .unwrap()
                .coarse_route_bank_reservation(first.executor, value)
                .unwrap()
    ...
    let second_ranges: Vec<_> = bank
        ...
        .map(|&value| {
            provider
                .residency()
                .unwrap()
                .coarse_route_bank_reservation(second.executor, value)
                .unwrap()
                .with_reservation_mut(|reservation, _| reservation.base_ptr())
        })
        .collect();
    ...
    assert_eq!(
        provider
            .route_residency_executor_status(first.executor)
    ...
    let drain = std::thread::spawn(move || {
        drain_artifacts(&drain_provider, first).expect("drain exact first scope");
    ...
    drain_artifacts(&provider, first).expect("idempotent first drain");
    assert_eq!(provider.residency().unwrap().route_reservation_count(), 1);
    assert!(provider.route_residency_scopes().contains(&second.executor));
    assert_eq!(
        provider
            .route_residency_executor_status(first.executor)
            .drain_calls,
    ...
    drain_artifacts(&provider, second).expect("drain exact sibling scope");
}

#[test]
#[ignore = "requires idle CUDA device with HOST_NUMA VMM support"]
fn repeated_retirement_serializes_behind_admitted_finalizer_and_cleans_once() {
    let _serial = SERIAL.lock().unwrap_or_else(|error| error.into_inner());
    let _gate = GateGuard::set(true);
    ...
    let census_before = provider
        .route_residency_retirement_census()
        .retirements_started;

    for cycle in 0..4 {
        let artifact = artifact_scope(&provider);
        let kernel = compile_real_qmoe(&provider, artifact, &graph, node);
        let interlock = Arc::new(RoutePrepareCommitInterlock::new());
        provider.install_route_prepare_commit_interlock_for_test(Arc::clone(&interlock));
        std::thread::scope(|scope| {
            ...
            let drain = scope.spawn(move || {
                ...
                let result = drain_artifacts(&drain_provider, artifact);
                returned_tx.send(()).unwrap();
                result
            });
            ...
            assert!(
                returned_rx
                    ...
                    .is_err(),
                "retirement must wait behind the exact-generation lifecycle gate, not race commit"
            );
            ...
            assert_eq!(
                finalize
                    ...
                    .expect("admitted finalizer commits before retirement"),
                ExecutorArtifactState::Required
            );
            returned_rx
                .recv_timeout(std::time::Duration::from_secs(1))
                .expect("retirement returns after commit releases the lifecycle gate");
            drain
                .join()
                .expect("drain thread")
                .expect("retire exact committed generation");
        });
        drop(kernel);
        assert!(
            provider
        ...
        assert_eq!(
            provider.residency().unwrap().route_reservation_count(),
            0,
            "cycle {cycle} must remove the committed reservation exactly once"
        );
        ...
        assert_eq!(
            census.retirements_started,
            census_before + cycle + 1,
            "cycle {cycle} retires exactly one committed reservation generation"
        );
        assert_eq!(census.prepared_rollbacks, 0);
    }

    let replacement = artifact_scope(&provider);
    let replacement_kernel = compile_real_qmoe(&provider, replacement, &graph, node);
    assert_eq!(
        finalize_artifacts(
            &provider,
            replacement,
    ...
    assert_eq!(provider.residency().unwrap().route_reservation_count(), 1);
    drain_artifacts(&provider, replacement).expect("drain replacement");
...
fn commit_authority_admits_exactly_one_concurrent_finalizer() {
    ...
    let artifact = artifact_scope(&provider);
    let _kernel = compile_real_qmoe(&provider, artifact, &graph, node);
    let interlock = Arc::new(RoutePrepareCommitInterlock::new());
    provider.install_route_prepare_commit_interlock_for_test(Arc::clone(&interlock));
    ...
    std::thread::scope(|scope| {
        ...
        let sibling_graph = &graph;
        let sibling_bank = &bank;
        let sibling = scope.spawn(move || {
            finalize_artifacts(
                ...
                sibling_graph,
                ExecutorArtifactReadinessEpoch::new(1),
                std::slice::from_ref(sibling_bank),
            )
        });
        assert_eq!(provider.residency().unwrap().route_reservation_count(), 1);
        ...
        assert_eq!(
            sibling
                .join()
                .expect("serialized sibling finalizer")
                .expect("sibling observes committed state"),
            ExecutorArtifactState::Required
        );
    });

    let status = provider.route_residency_executor_status(artifact.executor);
    assert_eq!(status.finalization_attempts, 1);
    assert!(matches!(
        status.outcome,
        Some(RouteResidencyInstallOutcome::Installed { banks: 4 })
    ...
    assert_eq!(provider.residency().unwrap().route_reservation_count(), 1);
    drain_artifacts(&provider, artifact).expect("drain committed exact scope");
}
...
fn retirement_registry_reclaims_churn_and_blocks_live_generation_aba() {
    ...
    for cycle in 0..8 {
        ...
        let generation = provider
            .route_residency_executor_status(artifact.executor)
            .reservation_generation
            .expect("generation");
        first_generation.get_or_insert(generation);
        drain_artifacts(&provider, artifact).expect("drain churn generation");
        assert!(
            provider
                .release_queue()
                .wait_until_idle(std::time::Duration::from_secs(30)),
    ...
    assert_eq!(
        ...
        .expect("fresh executor generation is isolated from old tombstones"),
        ExecutorArtifactState::Required
    );
    let replacement_generation = provider
        .route_residency_executor_status(replacement.executor)
        .reservation_generation
    ...
    drain_artifacts(&provider, replacement).expect("drain replacement generation");
...
fn readiness_absence_is_pending_and_concurrent_finalize_is_idempotent() {
    ...
    assert_eq!(
        ...
        ExecutorArtifactState::Pending(ExecutorArtifactPending::ProducerUnavailable { node })
    );
    ...
    assert!(results.contains(&ExecutorArtifactState::Required));
    ...
    assert_eq!(provider.residency().unwrap().route_reservation_count(), 1);
    drain_artifacts(&provider, artifact).expect("drain readiness generation");
}
...
fn default_off_retains_allocates_and_registers_nothing() {
    ...
    assert_eq!(
        provider
            ...
            .route_residency(),
        onnx_runtime_ep_api::ExecutorRouteResidencyConfig::Disabled
    );
    assert_eq!(
        finalize_artifacts(
            &provider,
    ...
    drain_artifacts(&provider, artifact).expect("drain default-off scope");
}
...
fn bqmoe_without_real_telemetry_and_catalog_contract_typed_declines() {
    ...
    assert_eq!(provider.residency().unwrap().route_reservation_count(), 0);
    drain_artifacts(&provider, artifact).expect("drain BQMoE scope");
}
...
fn overlapping_external_bank_properties_decline_before_reservation() {
    ...
    assert_eq!(provider.residency().unwrap().route_reservation_count(), 0);
    drain_artifacts(&provider, artifact).expect("drain overlap scope");
}
