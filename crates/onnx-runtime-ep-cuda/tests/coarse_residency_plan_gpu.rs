...
use onnx_runtime_ep_api::{LazyWeightBoundary, StaticProfileResidencyPolicy, plan_residency};
use onnx_runtime_ep_cuda::CudaExecutionProvider;
use onnx_runtime_ep_cuda::coarse_residency::{
    COARSE_RESIDENCY_PROFILE_ENV, apply_residency_plan_at_boundary,
};
...
fn real_transition_correctness_gate_on() {
    ...
    unsafe { std::env::set_var(COARSE_RESIDENCY_PROFILE_ENV, "1") };
    let outcome = apply_residency_plan_at_boundary(
        runtime,
        &residency,
        &plan,
    ...
    unsafe { std::env::remove_var(COARSE_RESIDENCY_PROFILE_ENV) };
