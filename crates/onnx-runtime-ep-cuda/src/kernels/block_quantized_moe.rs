...
use onnx_runtime_ep_api::{
    CaptureSupport, DeviceGraphResource, EpError, ExecutionProvider, ExpertExecutionPhase, Kernel,
    KernelConstantInput, KernelFactory, Result, SealedDeviceAllocation, TensorBacking,
    TensorMetadata, TensorMut, TensorView, WorkspaceLifetime, WorkspaceRequirement, WorkspaceView,
};
...
use crate::kernels::expert_route_telemetry::{
    H_COUNT, H_DEVICE, H_EPOCH, H_OVERFLOW, H_POISON, H_REQUEST,
};
...
pub(crate) struct BlockQuantizedMoEFactory {
    ...
    pub(crate) route_sources: Option<Arc<RouteTelemetryRegistry>>,
}
...
impl KernelFactory for BlockQuantizedMoEFactory {
    fn create(&self, node: &Node, input_shapes: &[Vec<usize>]) -> Result<Box<dyn Kernel>> {
        ...
        let route_activation = if let Some(registry) = self.route_sources.as_ref()
            && let Some(num_experts) = input_shapes.get(1).and_then(|shape| shape.get(1)).copied()
        {
        ...
        Ok(Box::new(SharedBlockQuantizedMoEKernel {
            kernel,
            route_activation,
        }))
    }
...
struct SharedBlockQuantizedMoEKernel {
    kernel: Arc<BlockQuantizedMoEKernel>,
    route_activation: Option<RouteSourceActivation>,
}

impl Kernel for SharedBlockQuantizedMoEKernel {
    fn prepare_constant_inputs(
        ...
        Arc::get_mut(&mut self.kernel)
            .ok_or_else(|| error("BlockQuantizedMoE was activated before constant preparation"))?
            .prepare_constant_inputs(constants, provider)?;
        self.arm_registered_route_source();
        Ok(())
    }

    fn shareable_constant_state(&self) -> Option<Arc<dyn std::any::Any + Send + Sync>> {
        self.kernel.shareable_constant_state()
    }
    ...
    fn adopt_shareable_constant_state(
        ...
        let adopted = Arc::get_mut(&mut self.kernel)
            .ok_or_else(|| error("BlockQuantizedMoE was activated before constant adoption"))?
            .adopt_shareable_constant_state(state)?;
        if adopted {
            self.arm_registered_route_source();
        }
        Ok(adopted)
    ...
    fn constant_input_override(&self, input_idx: usize) -> Option<TensorView<'_>> {
        self.kernel.constant_input_override(input_idx)
    }

    fn device_graph_resources(&self) -> Vec<DeviceGraphResource> {
        self.kernel.device_graph_resources()
    }
    ...
    fn arm_block_quantized_moe_traffic(&mut self, request_id: u32) -> Result<bool> {
        self.kernel
            .arm_route_telemetry(RouteTelemetryConfig {
                request_id,
                device_id: self.kernel.runtime.ordinal(),
                num_experts: self.kernel.geometry.experts,
                routes_per_row: self.kernel.attributes.k,
            })
    ...
    fn execute_with_workspace(
        ...
        self.activate_route_source();
        self.kernel
            .execute_with_workspace(inputs, outputs, workspace)
    }
...
impl SharedBlockQuantizedMoEKernel {
    fn arm_registered_route_source(&mut self) {
        let Some(activation) = self.route_activation.as_ref() else {
            return;
        };
        if self.kernel.arm_route_telemetry(activation.config).is_err() {
            self.route_activation = None;
        }
    }

    fn activate_route_source(&self) {
...
impl BlockQuantizedMoEKernel {
    ...
    fn disarm_route_telemetry_impl(&self) -> Result<()> {
    ...
    fn reset_route_telemetry_boundary_impl(&self) -> Result<()> {
    ...
    #[cfg(feature = "gpu-tests")]
    pub fn disarm_route_telemetry(&self) -> Result<()> {
        self.disarm_route_telemetry_impl()
    }

    #[cfg(not(feature = "gpu-tests"))]
    pub(crate) fn disarm_route_telemetry(&self) -> Result<()> {
        self.disarm_route_telemetry_impl()
    }
    ...
    #[cfg(not(feature = "gpu-tests"))]
    pub(crate) fn reset_route_telemetry_boundary(&self) -> Result<()> {
        self.reset_route_telemetry_boundary_impl()
    }
...
impl RouteTelemetrySource for BlockQuantizedMoEKernel {
...
mod claim_gate_tests {
    ...
    fn mixed_fc1_fc2_formats_are_claimable() {
        ...
        assert!(unsupported_reason(&final_layer, &[], &[]).is_none());
    }
