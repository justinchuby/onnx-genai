...
use onnx_runtime_ep_api::{
    CaptureSupport, EpError, ExpertExecutionPhase, Kernel, KernelFactory, Result, TensorMetadata,
    TensorMut, TensorView, WorkspaceLifetime, WorkspaceRequirement, WorkspaceView,
};
...
impl RouteTelemetrySource for BlockQuantizedMoEKernel {
    fn route_telemetry_snapshot(&self) -> Result<Option<TelemetrySnapshot>> {
        BlockQuantizedMoEKernel::route_telemetry_snapshot(self)
    }

    fn reset_route_telemetry_boundary(&self) -> Result<()> {
        BlockQuantizedMoEKernel::reset_route_telemetry_boundary(self)
    }
}
