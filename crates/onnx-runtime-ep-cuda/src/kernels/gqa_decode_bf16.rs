...
const ENTRY_PREFIX: &str = "gqa_decode_attention_bf16_h2pl";
...
/// `ceil(head_dim / 2 / 32)` `__nv_bfloat162` slots (2 dims each) in registers.
/// The decode kernel is specialized per half2-per-lane tier `H2PL = ceil(head_dim
...
/// (`GQA_DECODE_BF16_ENTRIES` 1..=8). `2 * 32 * MAX_H2PL == MAX_HEAD_DIM`.
...
#define GQA_MAX_H2PL 8   // __nv_bfloat162 slots per lane (widest tier); head_dim <= 2 * 8 * 32 == 512
...
template<int H2PL>
__device__ __forceinline__ void gqa_decode_bf16_impl(
...
// launcher selects `gqa_decode_attention_bf16_h2pl{N}` for
...
#define GQA_DECODE_BF16_ENTRIES(H2PL)                                         \
extern "C" __global__ void gqa_decode_attention_bf16_h2pl##H2PL(             \
    const __nv_bfloat16* __restrict__ query,                                 \
    const __nv_bfloat16* __restrict__ key,                                   \
    const __nv_bfloat16* __restrict__ value,                                 \
    __nv_bfloat16* __restrict__ output,                                       \
    ...
    gqa_decode_bf16_impl<H2PL>(query, key, value, output, total_lengths,      \
        batch, query_heads, kv_heads, query_seq, head_size, cache_capacity,   \
        group_size, scale, local_window, softcap, single_split_direct,        \
        split_fill);                                                          \
}

GQA_DECODE_BF16_ENTRIES(1)
GQA_DECODE_BF16_ENTRIES(2)
GQA_DECODE_BF16_ENTRIES(3)
GQA_DECODE_BF16_ENTRIES(4)
GQA_DECODE_BF16_ENTRIES(5)
GQA_DECODE_BF16_ENTRIES(6)
GQA_DECODE_BF16_ENTRIES(7)
GQA_DECODE_BF16_ENTRIES(8)
...
mod tests {
    ...
    /// General head-size coverage for the bf16 fused fast path. The decode
    ...
    #[test]
    fn bf16_decode_kernel_matches_reference_softmax_general_head_dims() {
        let Some(runtime) = runtime() else {
            eprintln!(
                "skipping CUDA GQA bf16 general head-dim parity test: CUDA runtime unavailable"
            );
            return;
        };
        if runtime
        ...
        {
            eprintln!(
                "skipping CUDA GQA bf16 general head-dim parity test: bf16 NVRTC headers unavailable"
            );
            return;
        }
        ...
        for head_dim in [64usize, 128, 256, 320, 384, 448, 512] {
            let scale = 1.0f32 / (head_dim as f32).sqrt();
            let mut state = 0x0B16_5EEDu64 ^ ((head_dim as u64) << 19);
            let mut next = || {
                state = state
                    .wrapping_mul(6364136223846793005)
                    .wrapping_add(1442695040888963407);
            ...
            eprintln!(
                "GQA bf16 decode head{head_dim} parity (h2pl={}): max_abs={worst_abs:.3e} max_rel={worst_rel:.3e}",
                super::h2_per_lane(head_dim)
            );
            // bf16 has ~8 mantissa bits (vs fp16's ~11), so tolerances are wider.
            assert!(
                worst_abs < 3e-2,
                "bf16 head{head_dim} decode diverged from reference softmax: max_abs={worst_abs:.3e}"
            );
            assert!(
                worst_rel < 2.5e-1,
                "bf16 head{head_dim} decode diverged from reference softmax: max_rel={worst_rel:.3e}"
            );
        }
    }
