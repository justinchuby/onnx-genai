...
impl Kernel for SkipLayerNormKernel {
    fn execute(&self, inputs: &[TensorView], outputs: &mut [TensorMut]) -> Result<()> {
        ...
        require_fused_float_dtype("SkipLayerNormalization", inputs, outputs)?;
...
mod tests {
    ...
    fn skip_layer_norm_bf16_matches_ort_literal_goldens() {
        // Independent golden generated by ONNX Runtime CPU from a tiny f32
        // com.microsoft::SkipLayerNormalization model, then RNE-rounded to bf16.
        ...
        let mut mean = Owned::zeros(DataType::BFloat16, &[1, 1]);
        let mut inv_std = Owned::zeros(DataType::BFloat16, &[1, 1]);
    ...
    fn skip_layer_norm_bf16_infinities_propagate_nan() {
        ...
        SkipLayerNormKernel { epsilon: 1e-5 }
            .execute(&[x.view(), skip.view(), gamma.view()], &mut [y.view_mut()])
            .unwrap();
