...
fn with_mlas_packed_caches<R>(f: impl FnOnce(&MlasPackedCaches) -> R) -> R {
    ...
    // becomes the tail expression. An early `return` in the first would read
    // more obviously but trips `needless_return` in the `cfg(test)` build --
    // which is now a default-feature build, so it reaches the `-D warnings`
    // lanes.
    #[cfg(test)]
    {
