//! Same-binary A/B: run a kernel family's native route and its MLAS route in
//! one process, on one host, under one load.
//!
//! # Why this is a module and not a test helper
//!
//! `docs/performance/ABSORBING_MLAS.md` records the method that works for
//! replacing an MLAS route with a native one: keep **both** implementations in
//! **one binary** behind an explicit toggle, then measure the gap in process CPU
//! time. Without that, the gap is asserted rather than measured, and a native
//! "win" that is actually a loss ships unnoticed.
//!
...
/// Whether this build linked MLAS at all. When false, the `*_mlas` entry points
/// return `None` and the differential tests degrade to native-vs-native
/// self-consistency rather than silently passing on nothing.
