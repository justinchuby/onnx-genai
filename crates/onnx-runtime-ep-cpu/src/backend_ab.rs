...
mod tests {
    ...
    fn assert_ab_family_has_both_halves(family: KernelFamily) {
        assert!(
            AB_COVERED.contains(&family),
            "{family} is tested as A/B-covered but absent from AB_COVERED"
        );
        assert!(
        ...
        let ran_mlas = match family {
            ...
            other => {
                panic!("{other} was added to AB_COVERED without an A/B entry point in this test")
            }
        };
        assert_eq!(
            ran_mlas,
    ...
    #[test]
    fn matmul_ab_family_has_both_halves() {
        assert_ab_family_has_both_halves(KernelFamily::MatMulF32);
    }

    #[test]
    fn softmax_ab_family_has_both_halves() {
        assert_ab_family_has_both_halves(KernelFamily::Softmax);
    }

    #[test]
    fn activations_ab_family_has_both_halves() {
        assert_ab_family_has_both_halves(KernelFamily::Activations);
    }

    #[test]
    fn attention_transpose_ab_family_has_both_halves() {
        assert_ab_family_has_both_halves(KernelFamily::AttentionTranspose);
    }

    #[test]
    fn every_ab_covered_family_has_a_named_reachability_test() {
        const TESTED: &[KernelFamily] = &[
            KernelFamily::MatMulF32,
            KernelFamily::Softmax,
            KernelFamily::Activations,
            KernelFamily::AttentionTranspose,
        ];
        assert_eq!(AB_COVERED, TESTED);
    }
