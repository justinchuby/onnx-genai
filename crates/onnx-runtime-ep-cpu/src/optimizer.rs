...
#[cfg(test)]
mod silu_fusion_tests {
    use super::*;
    use onnx_runtime_ir::{DataType, Dim, Node, NodeId};

    fn silu_graph(extra_consumer: bool) -> Graph {
        let mut graph = Graph::new();
        ...
        if extra_consumer {
            let other = graph.create_named_value("other", DataType::Float32, shape);
            graph.add_output(other);
            graph.insert_node(Node::new(
                NodeId(0),
                "Neg",
    ...
    /// `x * Sigmoid(x)` becomes the standard operator, not a private one.
    ///
    /// `Swish` is ONNX's own spelling of this since opset 24, so every provider
    /// recognises it. Emitting a contrib op instead is what previously made the
    /// MLX plugin decline these nodes and fragment its claim.
    #[test]
    fn fuses_single_consumer_pattern_into_standard_swish() {
        let mut graph = silu_graph(false);
        CpuSiluFusion.run(&mut graph, &PassContext::new()).unwrap();
        ...
        assert_eq!(fused.op_type, "Swish");
        assert!(
            fused.domain.is_empty(),
            "Swish is a default-domain operator; a private domain would hide it \
             from providers that know the standard one"
        );
        assert_eq!(
            fused.version,
            Some(24),
            "the node must carry the opset that introduced Swish, so a graph \
             exported against an older one stays truthful about its other nodes"
        );
    }

    /// A Sigmoid feeding anything else must not be folded away.
    ///
    /// Fusing would delete a value another node still reads, which changes what
    /// the graph computes rather than how fast it computes it.
    #[test]
    fn does_not_fuse_when_sigmoid_has_a_second_consumer() {
        let mut graph = silu_graph(true);
        CpuSiluFusion.run(&mut graph, &PassContext::new()).unwrap();

        assert_eq!(graph.num_nodes(), 3, "nothing may be fused away");
        assert!(graph.nodes.values().all(|node| node.op_type != "Swish"));
    }
}
