//! Capability-negotiated Phase-3a weight handles.
...
pub struct LazyWeight {
    ...
    pub regions: Vec<WeightRegionCatalog>,
...
impl LazyWeight {
    ...
    /// Materialize today's stock-EP behavior.
...
pub enum WeightHandleError {
    ...
    #[error("lazy weight requires at least one region catalog")]
    MissingRegions,
    #[error("unsupported: {0}")]
    Unsupported(String),
}
...
mod tests {
    ...
    use onnx_runtime_loader::{ExpertStorageOrder, ExpertTensorLayout};
    ...
    fn lazy() -> WeightHandle {
        let catalog = WeightRegionCatalog::for_mapped_tensor_view(
            DataType::Uint8,
            &[1, 1, 4],
            4,
            ExpertTensorLayout {
        ...
        WeightHandle::Lazy(
            LazyWeight::block_quantized_moe(vec![catalog], || Ok(resident())).unwrap(),
        )
    }
    ...
    fn nxrt_capability_preserves_lazy_block_quantized_moe_handle() {
        ...
        let WeightHandle::Lazy(mut weight) = lazy() else {
            unreachable!()
        };
        weight.resident_materializer = Arc::new(move || {
            counter.fetch_add(1, Ordering::Relaxed);
            Ok(resident())
        });
        let NegotiatedWeight::Lazy(weight) = WeightHandle::Lazy(weight)
        ...
        assert_eq!(weight.regions.len(), 1);
