...
/// generated by that crate's `generate_fixtures.py`. They are referenced in
/// place rather than copied: `.gitignore` blanket-ignores `*.onnx` with an
/// explicit allow-list per fixture path, so a copy here would be silently
/// untracked and the tests would pass locally but fail in CI.
