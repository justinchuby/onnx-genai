...
| Audio ASR (Nemotron) | [`nemotron-audio/`](nemotron-audio/) | ✅ | ❌ | No executable audio frontend or chunk scheduler. The decoder→joiner route requires `[B,640,T] → [B,T,640]`, but edges cannot express transpose. Generic typed port inventory is missing, and the conditional RNN-T feedback loop is not runtime-supported. |
