...
2. Add the smallest counterexample. Before fixing, require the intended assertion **and accurate
   ...
   For each mutation: count the target; change one condition; prove a changed hash or diff; run
   the named test with raw status; restore and prove the original hash. Unapplied looks robust.
...
One control per corpus: empty each reader and require its own named red.
For zero findings, use synthetic data outside the corpus or a permanent
...
1. **Fixture derives assertion data.** `syntheticBlockTable()` derived `pages_shared` from
   `refCounts`; another test compared a shared input/expectation variable with itself. Assert
   adversarial scopes differ first.
...
5. **Absence can masquerade as clean.** Negative assertions pass when a corpus or property is
   missing. Assert every corpus, key existence, and type before content.
