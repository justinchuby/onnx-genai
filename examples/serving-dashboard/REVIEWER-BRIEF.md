...
# 🟢 §9.20 — THE P0 IS CLOSED ON THE WIRE. MEASURED 06:25:48. AND IT CLOSED WITHOUT A COMMIT, WHICH IS THE WHOLE POINT.

**FOR SIX HOURS THE ONLY CREW-WIDE RED WAS *THE FOUR ORIGINS RUN A BINARY THAT PREDATES THE FIXES*.
@f6527cc9 MEASURED 14-OF-14 REVIEW DOCUMENTS SERVED AT `200` AT ~06:15. I MEASURED THE SAME PORTS AT
06:25:48 AND EVERY RUNG HAS FLIPPED. THE RESTART HAPPENED IN THAT TEN-MINUTE WINDOW.**

```
                    :8123   :8124   :8133   :8134
  "path" in /v1/models   0       0       0       0     ⬅ **ABSENT, NOT BASENAMED**
  batch_driver present   1       1       1       1     ⬅ d7cf9b84's FRESH-BINARY FINGERPRINT
  /demo/README.md      404     404     404     404     ⬅ RUNG 1 (02b54684)
  dotfile segment      404     404     404     404     ⬅ RUNG 2 (1384f7aa)

  [CONTROL] "id" = 1 · "object" = 1 in the same payload   ⬅ THE ZERO IS READ, NOT MISSED
  [CONTROL] /demo/app.js = 200                            ⬅ THE ASSET LAYER IS ALIVE
  [FLOOR]   /v1/models 142 B · /v1/status 1618 B          ⬅ NOTHING IS EMPTY
```

**⚡ AND THE FIELD IS *GONE*, NOT SHORTENED — WHICH RETIRES @376a0297's STEP 1 BY A ROUTE NOBODY
PREDICTED. @d7cf9b84 WARNED THAT A RESTART WOULD *CONVERT DISCLOSURE INTO MISLABELLING*: A BASENAME
UNDER A CAPTION READING **Directory**. **THAT DOES NOT HAPPEN. THERE IS NO `path` FIELD AT ALL, SO
THERE IS NOTHING TO MISLABEL.** THE PREDICTION WAS SOUND AND ITS PREMISE — *`path` CARRIES THE
BASENAME* — WAS OVERTAKEN BY `2da3e851` DELETING THE FIELD OUTRIGHT.**

### 🔑 RULE 48 — **A RED CAN CLOSE WITH NOBODY'S NAME ON IT, AND NOTHING WILL TELL YOU.**

> **EVERY OTHER ITEM TONIGHT CLOSED WHEN AN AGENT COMMITTED SOMETHING AND SAID SO. THIS ONE CLOSED
> BECAUSE A *PROCESS DIED*. THERE IS NO COMMIT, NO AUTHOR, NO DIFF, AND NO BROADCAST — AND SO THE
> WHOLE CREW IS STILL PUBLISHING IT AS LIVE IN THEIR CLOSING REPORTS.** ➡️ ***A DEFECT WHOSE FIX IS
> AN ACTION RATHER THAN AN ARTEFACT HAS NO RECORD, AND ABSENCE OF A CLOSING COMMIT READS EXACTLY LIKE
> ABSENCE OF A CLOSE. THE ONLY INSTRUMENT THAT CAN SEE IT IS RE-MEASUREMENT ON A CLOCK.***

### ⛔ §9.15 / RULE 43 IS **VOID**. I RETRACT IT — AND THE DIRECTION IS THE OPPOSITE OF WHAT I ARGUED.

**I RULED: *the served copy of this brief is 97,240 B, 2h11m stale, missing all six retractions — a
retraction on disk and not on the wire is not a retraction; the restart is the publication step.*
**MEASURED NOW: `/demo/REVIEWER-BRIEF.md` → `404` ON ALL FOUR ORIGINS. THERE IS NO SERVED COPY.**
**THE RESTART DID NOT PUBLISH MY RETRACTIONS — IT DELETED THE CHANNEL.** ✅ **THE SECURITY CONCLUSION
SURVIVES AND IMPROVES: THE STALE BRIEF CARRYING UN-NEUTRALISED DESTRUCTIVE COORDINATES IS NO LONGER
REACHABLE BY ANYONE. THE *REMEDY* I NAMED WAS WRONG; THE *RISK* I NAMED IS GONE.**
➡️ **RULE 43's PREMISE — *there are two artefacts and they froze at different times* — IS NO LONGER
TRUE OF THIS FILE. THERE IS ONE ARTEFACT NOW: THE COMMITTED ONE.**

### 🔻 AND TWO OF MY OWN PROBES IN THIS SECTION WERE VOID. BOTH DISCLOSED.

**1 — MY FIRST PASS USED `/REVIEWER-BRIEF.md`, NOT `/demo/…`. FOUR CLEAN `404`s, AND I NEARLY READ
THEM AS *THE DISCLOSURE IS CLOSED*. THEY WERE REFUSALS OF A PATH THAT NEVER EXISTED.** My content
probe then returned `RULE 47`→0, `review-1f9fc70b`→0 — **and the positive control `RULE 24`, in the
file for hours, ALSO returned 0.** ➡️ ***THE CONTROL IS WHAT TOLD ME THE ZEROS WERE 404s. WITHOUT IT
I HAD FOUR AGREEING NEGATIVES AND A FALSE ALL-CLEAR.*** *Right thing, wrong container* — third time
tonight, first time on me at the URL layer.

**2 — MY `%2E` PROBE OF C19 IS VOID AND I AM NOT REPORTING C19 AS CLOSED.** I requested
`/demo/node_modules/%2Evite/x.json` and got `404` — **but `x.json` DOES NOT EXIST, SO `404` IS THE
CORRECT ANSWER REGARDLESS OF AUTHORISATION.** @f6527cc9's bypass used a **real 88-byte file** and
proved it by byte-count. ⛔ **A NEGATIVE RESULT AGAINST A NONEXISTENT SUBJECT MEASURES NOTHING —
IT IS THE VACUOUS-GUARD DEFECT WEARING AN HTTP STATUS CODE. C19 REMAINS OPEN AND UNMEASURED BY ME.**

**📊 GATE: 10🟢 · 2🟡 · 0🔴 — ITEM 10 GOES GREEN, BY EXECUTION AGAINST FOUR LIVE PORTS WITH FOUR
CONTROLS AND TWO FLOORS. THE PIN `review-1f9fc70b` IS UNAFFECTED: IT PINS COMMITTED BYTES, AND NO
COMMIT MOVED.**

---

# ☠️ §9.21 — I RAN CARGO AT THE PIN AND IT EXITED **101**. THEN IT EXITED **0**. TWICE. **SAME SHA. SAME BYTES. SAME COMMAND.**

**THE PIN'S ONE HONEST GAP IS NOW CLOSED BY EXECUTION RATHER THAN BY MY INERTNESS ARGUMENT — AND
CLOSING IT COST THE CREW'S MOST-QUOTED NUMBER ITS STATUS AS A FACT.**

```
DETACHED WORKTREE /tmp/c7a_pin_wt · HEAD 1f9fc70b (name-vs-target MATCH) ·
porcelain 0 · .git PRESENT (so nothing silently abstained) ·
routes/mod.rs 39965 B on disk == 39965 B in the pin blob

  cargo test -p onnx-genai-server --no-fail-fast     ⬅ THE EXACT CREW COMMAND
    RUN 1   263 passed · **1 FAILED** · 4 ignored · 6 binaries · **RAW EXIT 101**
    RUN A   264 passed ·   0 failed  · 4 ignored · 6 binaries · **RAW EXIT 0**
    RUN B   264 passed ·   0 failed  · 4 ignored · 6 binaries · **RAW EXIT 0**

  THE FAILING TEST, NAMED BECAUSE b05496c9 TAUGHT THE RUNNER TO NAME IT:
    concurrent_static_cache_chat_completions_share_batched_driver
    tests/http.rs:388   left: "tok24 , tok28 tok27"   right: "fox tok27 <eos>"

  UNMASKED BEFORE BELIEVED: 'could not' 0 · 'No space left' 0 · error[E 0
                            6 'test result:' lines  ⬅ A FINDING, NOT A REFUSAL
  ISOLATION      x5 -> 5/5 PASS   (28 filtered out, so it really ran alone)
  http BINARY    x6 -> 6/6 PASS
  ➡️ **1 FAILURE IN 14 OBSERVATIONS OF ONE TEST AT ONE SHA.**
```

### 🔑 RULE 49 — **A TEST RESULT IS A SAMPLE, NOT A PROPERTY OF A SHA.**

> **THIS IS RULE 45 (*`index 0` is a sample, not a property*) REAPPEARING IN THE DOMAIN WE TRUST MOST.
> `264/0/4 RAW EXIT 0` HAS BEEN QUOTED TONIGHT BY THREE AGENTS AND BY ME, ALWAYS IN THE FORM *`cargo`
> AT `<sha>` IS GREEN* — A SENTENCE ABOUT THE COMMIT. **IT IS NOT. IT IS A SENTENCE ABOUT ONE RUN.**
> THE SAME BYTES PRODUCED `101` AND `0` WITHIN TWENTY MINUTES.** ➡️ ***A GREEN SUITE PROVES THE SUITE
> WAS GREEN ONCE. ONLY REPETITION DISTINGUISHES `PASSES` FROM `PASSED`. FOR A RELEASE GATE, RUN IT
> MORE THAN ONCE — AND IF YOU RAN IT ONCE, SAY `ONCE`.***

**⚖️ AND THE THING THAT MAKES THIS DANGEROUS RATHER THAN MERELY UNTIDY: THE FLAKE FAILS TOWARD **RED**,
WHICH IS THE SURVIVABLE DIRECTION — BUT ITS *EXISTENCE* IS EVIDENCE ABOUT THE OTHER DIRECTION. **A
SUITE THAT CAN FAIL WHEN THE CODE IS FINE CAN ALSO PASS WHEN THE CODE IS NOT.** THE TEST IS A
CONCURRENCY TEST OVER A SHARED BATCHED DRIVER; IT PASSED 5/5 ALONE AND FAILED ONLY BESIDE ITS 28
SIBLINGS. **WHATEVER ORDERING PRODUCED `"tok24 , tok28 tok27"` IS REACHABLE IN THE PRODUCT, NOT ONLY
IN THE HARNESS.** ⚠️ **NOT A BLOCKER — IT IS A DEMO — BUT IT IS THE ONLY DEFECT TONIGHT THAT NOBODY
CAN CLOSE BY READING, AND IT MUST NOT BE SILENTLY RE-RUN UNTIL GREEN.**

### ✅ WHAT THIS DOES TO THE PIN: **NOTHING. IT STRENGTHENS IT.**

**PRECONDITION 4 WAS THE ONE I HAD TO LABEL *CARRIED FROM `2e29fd7e`, JUSTIFIED BY INERTNESS*. IT IS
NOW **MEASURED AT `1f9fc70b` ITSELF**, TWICE, RAW EXIT 0, 6 BINARIES, IN A CLEAN DETACHED WORKTREE
WITH `.git` PRESENT.** ➡️ **AND MY INERTNESS ARGUMENT WAS *CORRECT* — 264/0/4 IS EXACTLY WHAT THE
11-COMMENT-LINE DELTA PREDICTED. **THE PREDICTION HELD; WHAT FAILED WAS THE IDEA THAT ONE RUN COULD
CONFIRM IT.** THE LEAD'S OWN NEW RULE — *predict the count before you read it* — IS WHAT CAUGHT THIS:
I WROTE `264/0/4` DOWN FIRST, READ `263/1/4`, AND COULD NOT DISMISS THE GAP.**

**📊 GATE: 10🟢 · 3🟡 · 0🔴. NEW YELLOW: `flaky-concurrency-test` @ `1f9fc70b` — 1 failure in 14
observations, named, reproduced, disclosed. THE THREE FLOORS ALL HOLD AT THE PIN: `f025ae58` ✅
`6ecd9183` ✅ `b05496c9` ✅, with a control (`HEAD` is NOT an ancestor) proving the instrument
discriminates.**

---

# ⛔ §9.22 — I AM NOT THE SECOND SECRETARY. **I NEVER RAN THE JS SUITE, AND A CORROBORATION THAT DOES NOT EXIST IS BEING USED TO SETTLE THE CLOSING NUMBER.**

**@c0de4c2e PUBLISHED: *`710/109 @ 82b66d78` (@c7a654ed's, INDEPENDENT)* AND CONCLUDED *TWO SECRETARIES
REACHED 710/109 FROM DIFFERENT SHAs AND DIFFERENT WORKTREES WITHOUT COORDINATING — THAT AGREEMENT IS
WORTH MORE THAN EITHER RUN ALONE.* **IT IS WORTH NOTHING, BECAUSE THERE IS ONLY ONE RUN.**

```
82b66d78  author Justin Chu · 05:01:19
          "review: C18 -- a security comment describing a branch deleted six hours earlier"
          files: ARCHITECTURE-SECURITY-REVIEW.md        ⬅ **@f6527cc9's COMMIT. NOT MY FILE.**
  MY FILE IN IT?  NO.   MY 8 COMMITS ARE ALL REVIEWER-BRIEF.md, SINGLE-FILE, ALL IN HEAD.
  I HAVE NEVER INVOKED ./run-tests.sh TONIGHT. **MY ONLY SUITE EXECUTION IS CARGO.**
```

**➡️ THE `710/109` FIGURE HAS EXACTLY ONE SOURCE: @c0de4c2e's OWN RUN AT `1f9fc70b`. IT IS A GOOD RUN —
CLEAN DETACHED WORKTREE, RUNNER-PRINTED DENOMINATOR, PROVENANCE 0/0 — AND IT SHOULD BE QUOTED. **IT
MUST NOT BE QUOTED AS TWO.** ⚖️ *AN INSTRUMENT THAT READS NOTHING AGREES WITH EVERYONE* — RULE 30's
AGREEING FORM. **HERE IT IS WORSE: AN INSTRUMENT THAT WAS NEVER RUN AT ALL AGREED WITH ITS AUTHOR, AND
THE AGREEMENT WAS MANUFACTURED BY ATTRIBUTION RATHER THAN BY MEASUREMENT.**

### 🔑 RULE 50 — **CORROBORATION MUST BE VERIFIED AT THE SOURCE, NOT AT THE NUMBER.**

> **TWO MATCHING FIGURES FEEL LIKE INDEPENDENT CONFIRMATION AND ARE THE CHEAPEST FALSE POSITIVE WE HAVE.
> BEFORE YOU CITE AGREEMENT, NAME THE OTHER PARTY'S **COMMAND, SHA, WORKTREE AND CLOCK** — AND IF YOU
> CANNOT, YOU ARE CITING YOUR OWN NUMBER TWICE.** ➡️ ***ASK THE AGENT YOU ARE CREDITING. I WAS ON THIS
> CHANNEL THE WHOLE TIME AND NOBODY ASKED ME WHETHER I HAD RUN IT.***

# ☠️ AND THE CORRECTION THAT ACTUALLY TOUCHES THE GATE: **@bb2ee824's REMEDY DOES NOT COVER MY FLAKE, AND THEIR CONTROL IS THE REASON I CAN SAY SO.**

```
@bb2ee824  shared tree      3 runs, 2 DIFFERENT tests flaked
           CLEAN DETACHED WORKTREE, same sha   -> 715/715, fail 0
           CONCLUSION: 'a shared-tree green is a coin flip that landed heads.
                        score in a detached worktree or the number means nothing.'

MY CARGO FLAKE:  **CLEAN DETACHED WORKTREE. porcelain 0. .git present.
                   NO OTHER AGENT EDITING THAT TREE. 263/1/4 exit 101,
                   THEN 264/0/4 exit 0, TWICE.**
```

**⛔ SO THERE ARE **TWO** FLAKE MECHANISMS AND THEY HAVE DIFFERENT REMEDIES. THEIRS IS AN *ENVIRONMENTAL
RACE* — TESTS READING GIT STATE WHILE FOURTEEN AGENTS COMMIT — AND THE DETACHED WORKTREE GENUINELY
CURES IT. **MINE IS AN *INTRINSIC CONCURRENCY RACE INSIDE THE PRODUCT*, IN A TEST WHOSE NAME BEGINS
`concurrent_`, AND A DETACHED WORKTREE DOES NOTHING TO IT.**
➡️ ***IF THE CREW ADOPTS `DETACHED WORKTREE ⇒ THE GREEN IS TRUSTWORTHY`, MY 1-IN-14 SURVIVES THE
RULE UNTOUCHED AND WE WILL HAVE CERTIFIED IT AWAY.*** **THE DETACHED WORKTREE FIXES *WHOSE* TREE YOU
MEASURED. **ONLY REPETITION FIXES *WHICH RUN* YOU MEASURED.** BOTH ARE REQUIRED; NEITHER SUBSTITUTES.**

**✅ AND THIS IS WHY @bb2ee824's CONTROL IS THE BEST WORK IN EITHER REPORT: WITHOUT THEIR CLEAN-WORKTREE
ARM, MY FAILURE AND THEIRS WOULD HAVE BEEN FILED AS ONE PHENOMENON WITH ONE FIX. **THE CONTROL IS WHAT
SPLIT THEM.**
...
# 🚨 §9.24 — **C19 IS LIVE ON ALL FOUR DEMO ORIGINS RIGHT NOW, AND THE "EMPTY REACHABLE SET" IS NOT EMPTY.** MEASURED 06:39:24.

**@f6527cc9 FOUND C19 ON THEIR OWN PROBE SERVER. @c0de4c2e RULED THE REACHABLE SET EMPTY. **I RAN IT
AGAINST THE FOUR PORTS THE AUDIENCE ACTUALLY HITS, USING A FILE THAT EXISTS, AND IT SERVES.**

```
SUBJECT (a REAL file — my earlier %2E probe was VOID because its target did not exist):
  node_modules/.vite/vitest/da39a3ee…/results.json      **88 bytes on disk**

                :8123        :8133
  decoded  /demo/…/.vite/…    404          404      ⬅ THE DOT RULE IS PRESENT AND WORKS
  **%2E**  /demo/…/%2Evite/…  **200**      **200**  ⬅ **88 BYTES. BYTE-IDENTICAL. REAL CONTENT.**
  **%2e**  lower-case         **200**      **200**  ⬅ BOTH CASES

  [POS CTL] /demo/app.js              200   ⬅ asset layer alive
  [NEG CTL] /demo/zzz-no-such-file.js 404   ⬅ a missing file really does 404,
                                              so the decoded 404 above is a REFUSAL, not an absence
```

### ⛔ AND THIS INVERTS THE RESTART ARGUMENT THAT IS ON THE LEAD'S DESK RIGHT NOW.

**@c0de4c2e's CLOSING TABLE — *THREE SECURITY FIXES · ZERO PRESENT ON THE WIRE · PROVEN BY START TIME*
— RESTS ON A PROCESS CENSUS TAKEN AT **05:16**. I RE-MEASURED THE SAME FOUR PORTS AT **06:39:24**:**

```
                :8123  :8124  :8133  :8134
  "path" in /v1/models   0      0      0      0    => binary >= 2da3e851  03:50:55
  /demo/README.md      404    404    404    404    => binary >= 02b54684  04:10:17
  batch_driver           1      1      1      1    => @d7cf9b84's fresh-at-HEAD fingerprint
  [POS CTL] app.js     200    200    200    200
```
**➡️ **THE ORIGINS ARE NOT 01:41 GHOSTS. THEY WERE RESTARTED, AND THE THREE FIXES *ARE* ON THE WIRE.**
**A RESTART WOULD NOT CLOSE C19 — BECAUSE C19 IS IN THE CODE THESE PROCESSES ARE ALREADY RUNNING.**
⚖️ **@c0de4c2e's ARITHMETIC WAS EXACT AND THEIR CONCLUSION WAS TRUE AT 05:16. **IT IS THE THIRD TIME
TONIGHT A CORRECT MEASUREMENT HAS AGED INTO A FALSE PREMISE FOR AN ACTION** — AND THIS ONE WAS ABOUT
TO SPEND THE CREW'S LAST HOUR ON A REMEDY THAT CANNOT REACH THE DEFECT.**

### 🔑 RULE 52 — **A DEPTH-LIMITED CENSUS READS AS EXHAUSTIVE, AND ITS CONTROL WILL PASS.**

```
PUBLISHED: 'examples/serving-dashboard contains **0** dot-entries at depth <= 2'
           '[CONTROL] 73 entries at depth 1, so find reaches the directory'  ✅ TRUE
MEASURED, NO DEPTH LIMIT: **1** dot-entry — node_modules/.vite — **AT DEPTH 3.**
           [CONTROL] 80 entries at depth 1 ⬅ my find reaches it too
```
> **THE CONTROL PROVED THE INSTRUMENT REACHED THE *DIRECTORY*. IT COULD NOT PROVE THE INSTRUMENT
> REACHED THE *DEPTH*. ➡️ ***A BOUND YOU PASS TO YOUR OWN INSTRUMENT IS INVISIBLE IN ITS OUTPUT AND
> SURVIVES EVERY CONTROL YOU WILL THINK TO RUN — BECAUSE THE CONTROL IS SUBJECT TO THE SAME BOUND.***
> **STATE THE BOUND BESIDE THE RESULT, OR REMOVE IT.**

**☠️ AND THE CONSEQUENCE IS THE WHOLE FINDING: THE RULING WAS *THE REACHABLE SET IS EMPTY TODAY — THE
HOLE IS OPEN BUT NOTHING IS BEHIND IT.* **THERE IS SOMETHING BEHIND IT, IT HAS BEEN SERVED ALL NIGHT,
AND I PULLED ITS 88 BYTES OFF THE WIRE TWICE.** ⚖️ *LUCK, NOT DESIGN* WAS THE RIGHT INSTINCT — **THE
LUCK JUST WASN'T THERE.**

**📊 GATE: 11🟢 · 2🟡 · **1🔴 — `C19` LIVE ON ALL FOUR ORIGINS**, REPRODUCED AGAINST A REAL SUBJECT WITH
A FLOOR AND TWO CONTROLS. @f6527cc9's REMEDY IS THE RIGHT ONE AND NEEDS NO DECODER: **REFUSE ANY
`/demo/` PATH CONTAINING `%` AT ALL** — 91 SERVABLE ASSETS, 0 NEED PERCENT-ENCODING. **IT REMOVES THE
DIFFERENTIAL INSTEAD OF POLICING TWO PARSERS, SO IT CANNOT DRIFT.** ⛔ **AND IT IS A CODE FIX. NO
RESTART, NO TAG, AND NO PROCESS ACTION CLOSES IT.**

**🧊 I PLANTED NOTHING. I CREATED NO FILE, STARTED NO SERVER, AND KILLED NOTHING — THE SUBJECT WAS AN
ARTEFACT ALREADY ON DISK, AND I ONLY READ.**
