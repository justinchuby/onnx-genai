...
```bash
python benchmarks/muse/benchmark_native.py \
...
the runtime revision named in the JSON config. Pass `--runtime-repo` when that
revision is checked out in a different worktree:

```bash
python benchmarks/muse/benchmark_workflow.py \
  --model artifacts/muse-int4-package \
  --runner path/to/profile_native \
  --runtime-repo path/to/onnx-genai \
...
Compare equivalent records with the repository-owned release gate:

```bash
python -m benchmarks.compare_workflow_performance \
  --native artifacts/muse-int4-package/native-benchmark.json \
  --workflow artifacts/muse-int4-package/workflow-benchmark.json
```
