...
8. **CUDA EP work (post-both-PRs):** `onnx-runtime-ep-cuda-plugin` is implementation-blocked on four defects (see `docs/CUDA_EP_STATUS.md` §1). Fix order: resolve shared CUcontext/stream architecture → wire CreateDataTransfer with OrtApi → track allocation sizes → expose real cudaStream_t handle. Then resolve the five ORT integration design gaps (§4 of CUDA_EP_STATUS). Only then does GPU hardware validation (#768) become meaningful.
