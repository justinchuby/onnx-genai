...
See `docs/NXRT_ABI.md` §6 and `crates/onnx-runtime-ep-nxrt-abi/src/status.rs`.
...
it was broken. See `docs/CUDA_EP_STATUS.md` §1 for the four specific defects.
...
- **CUDA EP:** The `onnx-runtime-ep-cuda-plugin` shim crate exists (M2) but is
  **implementation-blocked**. Four defects prevent correct operation on any host,
  GPU or not (see `docs/CUDA_EP_STATUS.md` §1). The plugin now fails closed: zero
...
8. **CUDA EP work — done in #832.** The four defects are fixed and the plugin was validated on an NVIDIA H200: ORT discovers 8× `cuda_ep` devices and executes single- and multi-node graphs on-device. See `docs/CUDA_EP_STATUS.md` §1; #768 now tracks only the residual items in §7.
