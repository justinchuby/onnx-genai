...
Native now has recorded hermetic parity for the covered workflow-language
surface: single-pass components; nested branch/loop/state; autoregressive and
speculative state; VLM; guided diffusion; masked-token; codec, TTS, and speech;
and video workflows. These records compare the same package against ORT and
assert that Native constructed zero ORT component sessions and made native
component calls. They establish neither universal native operator/kernel or
model compatibility nor real-package coverage. Unsupported native
operator/dtype/provider requirements fail closed; ORT execution islands remain
an ORT-only optimization and their absence on Native is not a functional
fallback or a performance comparison.
...
| Image multimodal preprocessing + VLM | Yes | Yes | Yes | Recorded artifact | Yes | Recorded fixture | No accepted record | [`vlm_workflow_parity`](../../crates/onnx-genai-engine/tests/native_workflow_parity.rs) records hermetic Native/ORT parity. Hosted Qwen vision packages record ORT execution, not a native real-package result. |
...
| Video-producing workflow/diffusion | Yes | Yes | Yes | Recorded fixture | Yes | Recorded fixture | No accepted record | [`video_workflow_parity`](../../crates/onnx-genai-engine/tests/native_workflow_parity.rs) records hermetic Native/ORT parity. It does not establish native compatibility for production video packages. |
| Audio preprocessing + multimodal/audio workflow | Yes | Yes | Yes | Recorded artifact | Yes | Recorded fixture | No accepted record | [`codec_workflow_parity`](../../crates/onnx-genai-engine/tests/native_workflow_parity.rs), [`tts_workflow_parity`](../../crates/onnx-genai-engine/tests/native_workflow_parity.rs), and speech fixtures record hermetic Native/ORT parity; this is not a general native real-audio claim. |
...
| Nested/general workflow IR | Yes | Yes | Yes | Recorded fixture | Yes | Recorded fixture | No accepted record | [`nested_loop_branch_and_state_parity`](../../crates/onnx-genai-engine/tests/native_workflow_parity.rs) records Native execution of nested control flow and state. This is selected fixture coverage, not a universal operator or package corpus. |
...
| EAGLE-3 | Yes | Yes | Yes | No record | No | No record | No accepted record | Implementation exists, but [`real_chained_proposer_matches_target_and_accepts_and_rejects`](../../crates/onnx-genai-engine/tests/chained_proposer_real.rs) returns success without executing when its package variable is absent. Native explicitly rejects this legacy EAGLE-3 mode; that exclusion does not apply to general or nested workflows. |
...
| Workflow policy interpreter/sampler | Yes | Yes | Yes | Recorded artifact | Yes | Recorded fixture | Fail (current scoped gate) | Hermetic single-pass and masked workflow parity establish functional Native policy-component execution, not an optimized sampler. [The current measured baseline](../benchmarks/2026-08-21-mobius-workflow-conformance.md#current-measured-baseline) records `0.903x` direct-composite throughput and min-p TTFT regression, and excludes production KV/per-row serving. |
