...
(`crates/onnx-genai-engine/src/pipeline/workflow.rs:3070-3176`) validates one
`uint8` rank-1 `encoded` input and calls `preprocess_encoded([encoded_bytes])`
with a one-element array (`workflow.rs:3135`). The multi-image capability of the
...
(`workflow.rs:1718-2332`, `invoke_onnx_component` at `workflow.rs:2570`), so an
...
(`crates/onnx-genai-engine/src/pipeline/workflow.rs:1134-1195`), so a symbol names
...
- **An emit MUST NOT trim a padded output.** `WorkflowNode::Emit` carries an
  ...
  (`workflow.rs:4852-4882`). If the same output also declares `padding`, the two
  describe one raggedness and cannot both be right. The emit publishes a shorter
  tensor; the `valid_lengths` companion — which this section makes the *sole*
  account of the padding and [§4](#4-strict-token_packed-validation) rule 8
  requires the workflow to publish — then measures a tensor that was never
  delivered. Either the caller holds lengths that overrun its payload, or the
...
- **`token_packed` needs no separate rule, and gets none.** An emit carrying
  ...
  document whose only remaining message explains the wrong defect.
- **The exclusion is keyed on `padding`, not on the layout, and a packed value
  may legally pad.** [§4](#4-strict-token_packed-validation) rule 7 forbids a
  `padding` entry only on the dimension a layout *packs*, and calls an entry on
  any **other** dimension the normal composition of the three kinds of
  raggedness. A value that is `token_packed` on axis 0 and padded on two further
  dimensions is therefore an ordinary declaration — row 25 of
  [§9](#9-e2e-acceptance-matrix) requires one — and it is the case with the
  **most** extents an emit-level trim can misdescribe. Scoping the exclusion to
  layouts that declare a request axis would let exactly that value keep a trim
  contradicting two published length vectors, and would leave its author holding
  a message about a missing row axis when the defect is a second account of
  raggedness. An earlier revision of this section claimed the exclusion was
  "load-bearing only where a request axis exists"; **that is withdrawn**. The
  correction came from the implementation, which reports both refusals for the
  doubly-padded packed case and which fails its own test when the scoping this
  document implied is inserted.
...
- **P2 — validation. Shipped in
  ...
  **both** the pre-existing no-row-axis rule
  ...
  refused without it — and that narrowing loses the only message naming the
  length vectors the trim contradicts. The two negatives above differ only in the
  `valid_length`'s rank, which the *interpreter* dispatches on and the load-time
  predicate does not; an implementation **MAY** therefore cover them with one
  fixture, and this list does not require redundancy it cannot justify. Both
  positives **MUST** be guarded against becoming vacuous, since a control that
  stops matching passes forever. Depends on P1.
...
**Every row that requires a rejection is satisfied only by asserting the message
that names the rule, never by observing that something was rejected.** A fixture
built to violate one rule usually violates others incidentally, so an
outcome-only case is precise by accident and nothing holds that: narrow the rule
away and some other refusal keeps the case green while it has stopped testing
what it names. The failure is one-directional and therefore invisible in review —
**the outcome survives the narrowing and only the message is lost** — which is
why row 25 asserts *both* messages where two rules fire, and why the scoping
error it was written against passed the earlier wording. That earlier wording did
not merely fail to catch the defect; it was *satisfied by* it, since it demanded
refusal by the pre-existing rule and the narrowed validator delivers exactly
that. A criterion a defective implementation passes as cleanly as the correct one
is worse than an absent criterion, because it converts review into evidence.
...
| 27 | **`request_expanded` participates.** A component with a `request_expanded` port at factor > 1 grouped alongside packed ports. | Ownership is arithmetic — entry `i` belongs to row `i / factor` — no companions are declared or required, footprint is charged as `rows × factor`, and a declaration that is both request-expanded and packed on one axis is rejected at load with the message naming that pair — not merely refused, since a request-expanded packed declaration trips other rules too and an outcome-only case would stay green after this one was narrowed away. | P2, P4 |
