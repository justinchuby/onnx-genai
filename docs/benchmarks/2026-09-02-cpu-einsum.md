...
The lock must wrap the actual `cargo bench --bench einsum` invocation so
...
custody. Replace `your_name` with a stable identifier accepted by
`hostlock.sh`, choose a physical-core budget valid for the machine, and use a
