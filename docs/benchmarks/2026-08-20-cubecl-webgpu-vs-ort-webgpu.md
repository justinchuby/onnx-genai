...
## 更正:正文那张表几乎没有测到 kernel

这是本文最重要的一条更正,放在这里是因为它改变了上面所有比值的**含义**,
而不是它们的数值。

正文的三个 matmul cell 最大只有 16.8 MFLOP。warmed profile 显示
`medium_gemm/f32` 的 wall 是 **962us** 而 ORT 归给节点的 dur 只有 **8-9us**
——99% 的时间不在 kernel 里。更直接的证据是:三个 cell 的工作量差 32 倍,
时间只差 1.5 倍。时间不随工作量变化,是固定开销主导的特征。

所以正文那张表衡量的是 **per-Run 固定路径**,不是 GEMM 实现。

### 加了工作量阶梯之后

已发布的三个形状原样保留(否则那些数字失去可复现性),另加两个:

| cell | 形状 | FLOP | 相对 medium |
|---|---|---|---|
| `medium_gemm` | (32, 512, 512) | 16.8 M | 1x |
| `huge_gemm` | (512, 1024, 1024) | 1.07 G | 64x |
| `xl_gemm` | (1024, 2048, 2048) | 8.6 G | 512x |

优化前的比值:**1.64 → 2.56 → 3.34**。比值随工作量单调上升,说明确实走出了
固定开销主导区,后两个 cell 测到的是 kernel。

用 `huge → xl` 的**增量**消掉固定开销,得到纯 kernel 吞吐:

| | Δt | ΔFLOP | 吞吐 |
|---|---|---|---|
| CubeCL | 19250us | 7.53 G | **391 GFLOP/s** |
| 官方 | 5379us | 7.53 G | **1400 GFLOP/s** |

分母检查(这一步不能省):M4 Max 的 f32 峰值约 9 TFLOP/s,于是我们是峰值的
4%、官方是 15%。对一个 naive tiled 实现和一个 vectorized 实现,这两个数都
落在合理量级——如果算出来是 40% 或 0.4%,那该怀疑的是仪器而不是系统。

**结论:真正的差距在 matmul kernel 本身(3.6x),不在 per-Run 开销。** 这与
正文小形状表给人的印象相反,而正文表并没有错——它只是在回答另一个问题。

### 优化之后

`matmul_regtiled`(寄存器分块,见 `kernels/matmul.rs`)之后,`xl_gemm/f32` 的
cubecl:official 比值:

| 版本 | 比值 |
|---|---|
| `matmul_tiled`,每 thread 一个输出 | 3.34x |
| 寄存器分块 4x4 | 2.04x |
| 4x4 + `lhs_tile` 转置 | 2.06x(无收益) |
| 寄存器分块 8x8 + 转置 | 1.77x |
| 8x8 + vec4 全局访存 | **1.55x** |

**仍然没有超过官方。** 累计提速 2.2x,但还差 1.55x。

vec4 那一行的三次独立测量是 1.545 / 1.543 / 1.556(median 1.55x),离散度
0.8%。同三次里官方 arm 的绝对值漂了 43%(huge 1714us→2443us),而比值只动
了 3%——这一节坚持只报比值的理由就在这里。

### 一个被否定的假设

vec4 之前 `huge_gemm` 的比值(1.84x)差于 `xl_gemm`(1.77x),当时的猜测是
huge 只有 4x8=32 个 cube,填不满 40 个核。**这个猜测是错的。** vec4 之后两
者收敛到 1.54x / 1.55x,而 vec4 完全没有改变 cube 数或占用率,只改了每次全
局访存搬多少字节。所以那 0.07 的差是访存效率,不是占用率。

代价是:如果当时按这个假设去做两级 tiling,会花掉几天换来零收益。

### 天花板

用 huge→xl 的增量消掉固定开销后的纯 kernel 吞吐(两次干净测量):

| | GFLOP/s | 占 M4 Max f32 峰值(~9 TFLOP/s) |
|---|---|---|
| CubeCL(vec4 前) | ~730 | 8% |
| CubeCL(vec4 后) | ~930 | 10% |
| 官方 WebGPU EP | ~1450 | 16% |

剩下的 1.55x 不太可能靠继续调分块常量拿到:

- **double buffering 走不通**——`BM*BK + BK*BN` 在 BK=16 时正好顶满 16 KiB
  的 baseline 上限,再开一份 buffer 放不下。
- 全局访存已经是 vec4,是这个 API 下的上限。
- 官方那份 GEMM 是长期调优过的实现。要追上大概需要 subgroup 级别的原语,
  那是另一个量级的工作,不属于"稍微优化一下下"。

**给出这个天花板比再挤 5% 更有用**:它说明想在 GEMM 上赢官方,应该换的是
算法层级(subgroup/cooperative matrix),而不是继续拧现在这个 kernel。

### 这一节的绝对时间为什么一个都没写

因为它们不可用。官方 arm 的代码在整个过程中一个字节都没变,而它的
`xl_gemm` 中位数在同一天的几轮里从 **6441us 漂到 8807us**(+37%)。任何跨轮
的绝对值比较都会把这 37% 读成优化效果。

只有**同一次运行内**的 cubecl:official 比值是可用的:官方 arm 是天然对照组,
比值把环境因子约掉了。上表每一行都来自单次运行内的配对采样。

污染源之一已经定位:`ReportCrash` 一度占 **89% CPU**,而喂给它的崩溃正是本
仓库那个哨兵自检故意触发的 C++ abort。工具链会污染自己的测量环境。
