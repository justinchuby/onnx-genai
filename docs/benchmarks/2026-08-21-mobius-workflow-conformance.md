...
This is the dated Mobius producer evidence that accompanied the canonical
metadata migration. The current runtime-wide methodology and acceptance gate
are defined in
...
[`benchmarks.workflow_performance.compare_performance`](../../benchmarks/workflow_performance.py)
contains:
...
The ONNX GenAI branch Mobius validated against was rebased as its own base moved, so
...
once the unreferenced object is collected. Only Mobius
[`.github/workflows/main.yml`](https://github.com/onnxruntime/mobius/blob/main/.github/workflows/main.yml)
...
Only the metadata is committed in Mobius. The graphs and the adapter weight file are a
deterministic function of
[`tests/generate_onnx_genai_validation_packages.py`](https://github.com/onnxruntime/mobius/blob/main/tests/generate_onnx_genai_validation_packages.py),
