...
| ~~`crates/mlas-sys/build.rs`; `crates/onnx-runtime-ep-cpu/Cargo.toml`~~ **(resolved)** | Was: the opt-in `mlas` feature was x86-64-Linux-shaped and would fail on Windows MSVC or macOS. `mlas-sys` now compiles MASM for `x86_64-pc-windows-msvc` and `aarch64-pc-windows-msvc`, clang/GAS for Linux and Apple on both architectures, and assembles the aarch64 GAS `.S` sources in four ISA groups (baseline, `+fp16`, `+i8mm`, `+bf16`) so cross-compilation to `aarch64-unknown-linux-gnu` links. That last part was a live break: `build.rs` previously compiled only the MSVC ARM64 `.asm` sources, leaving `MlasSgemmKernelZero`, `MlasGemvFloatKernel`, `MlasConvSymKernelNeon` and ~30 more declared and called but never defined — invisible because `scripts/check_cross_compile.sh` ran `cargo clippy`, which never links. It now also *builds* the `onnx-runtime-ep-cpu` test binaries for aarch64. Unsupported architectures `panic!` in `build.rs` with the supported list instead of producing an empty archive. `mlas` is an **opt-in** feature (off by default) per `docs/performance/CPU_MLAS_MIGRATION.md`, so the feature-specific CI this row asked for is the explicit `--features mlas` lanes in `.github/workflows/ci.yml` (the plugin cdylib builds and the MLAS ORT gate), not the default lanes. The full `onnx-runtime-ep-cpu` lib suite (1314 tests) now passes on `aarch64-unknown-linux-gnu` under `qemu-aarch64-static` with `--features mlas`, and the same fix turned the long-red `Rust coverage (macOS arm64)` lane green — it had been failing on `main` at `Build the shipped CPU plugin cdylib with MLAS` with ~40 undefined `_MlasSgemmKernel*`/`_MlasConvSym*` symbols for `architecture arm64` ever since #1115 added that step. | Remaining: no Windows-ARM64 runner executes the MLAS *kernels*, only builds them. macOS arm64 now runs the EP suite, but no lane asserts numeric MLAS parity on Windows ARM64. |
