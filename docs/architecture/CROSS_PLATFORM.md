...
| ~~`crates/mlas-sys/build.rs`; `crates/onnx-runtime-ep-cpu/Cargo.toml`~~ **(resolved)** | Was: the opt-in `mlas` feature was x86-64-Linux-shaped and would fail on Windows MSVC or macOS. `mlas-sys` now compiles MASM for `x86_64-pc-windows-msvc` and `aarch64-pc-windows-msvc`, clang/GAS for Linux and Apple on both architectures, and selects the aarch64 fp16 sources into their own `-march=armv8.2-a+fp16` group so cross-compilation to `aarch64-unknown-linux-gnu` links. Unsupported architectures now `panic!` in `build.rs` with the supported list instead of producing an empty archive. `mlas` is a **default** feature as of `docs/performance/CPU_MLAS_MIGRATION.md`, so every default lane in `.github/workflows/ci.yml` is the feature-specific CI this row asked for. | Remaining: no Windows-ARM64 or macOS runner executes the MLAS *kernels*, only builds them. Add a numeric parity assertion on those two lanes. |
