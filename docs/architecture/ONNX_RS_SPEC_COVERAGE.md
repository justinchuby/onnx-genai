...
| P0 | ⚠️ **The operator schema catalog now contains 99 high-value operators / 111 versioned entries.** Issue #75 adds 16 kernel-backed operators: `BitShift`, the four `Bitwise*` ops, `Dropout` (13/22), `EyeLike` (9/22), `GroupNormalization` (18/21), `Hardmax`, `IsInf`, `IsNaN`, `LpNormalization`, `PRelu`, `Selu`, `ThresholdedRelu`, and `Unique`. | Common transformer/CNN/indexing/shape-computation graphs validate, and this batch closes 16 concrete CPU/CUDA kernel-without-schema gaps. Sequence/optional schemas, broader control flow, and ONNX-ML remain incomplete. |
...
| P1 | ⚠️ **The shape-inference registry now contains 196 operators / 236 versioned entries.** A later Issue #75 batch adds 16 more standard tensor operators: `ConvTranspose` (deconvolution formula, `output_shape`/`SAME_*` handling), `GridSample`, `Einsum` (equation-driven, with ellipsis broadcasting), the `Celu`/`Shrink`/`Mish`/`HardSwish` activations, `LRN`, `MeanVarianceNormalization`, `ReverseSequence`, `RandomNormal`/`RandomUniform` and their `*Like` variants, `Bernoulli`, and `Multinomial`. A second Issue #75 batch adds 6 more standard tensor operators: `Det` (batched determinant, trailing square-matrix collapse), `LpPool` and `GlobalLpPool` (Lp windowed/global pooling, reusing the spatial formulas), `MaxUnpool` (transpose spatial formula or explicit `output_shape`), `Col2Im` (column-to-image fold), and `CenterCropPad` (center crop/pad along selected axes). A third Issue #75 batch adds 9 more standard tensor operators: the signal transforms `DFT` (opset 17 attribute-axis and opset 20 input-axis forms, `dft_length`/`onesided`/complex-`2` handling) and `STFT` (frame/bin counts from `frame_step`, `frame_length`, or the `window` length); the generators `MelWeightMatrix` (`floor(dft_length/2)+1` × `num_mel_bins`) and the cosine-sum windows `HannWindow`/`HammingWindow`/`BlackmanWindow` (rank-1 `[size]`); `AffineGrid` (2-D `[N, H, W, 2]` / 3-D `[N, D, H, W, 3]` sampling grids from the target `size`); and the loss operators `NegativeLogLikelihoodLoss` and `SoftmaxCrossEntropyLoss` (per-sample tensor under `reduction = none`, otherwise a scalar, plus the optional `log_prob` output). Sequence/optional/map container operators remain deferred pending an SSA type-model change. Symbolic/unknown extents degrade to stable fresh symbols rather than fabricated constants, and every rule is `since_version`-gated. | Kernel-backed tensor operators are substantially covered; full ONNX shape-inference parity still depends on container-valued sequence/optional types (the IR carries only tensor `dtype`+`shape` on graph values) and control-flow rules beyond `If`. |
...
| `TensorShapeProto` | `dim`(1) | ✅ | ⚠️ rank is represented; the shape registry covers 196 operators / 236 versioned entries, while the incomplete container/control-flow catalog limits full shape coverage | ✅ tensor/sparse signatures | ✅ | ✅ |
...
| Standard operator schemas through opset 25 | ✅ node serialization | ✅ node syntax | ⚠️ 99 high-value operators / 111 versioned entries | ✅ for the registered catalog; ⚠️ catalog incomplete |
