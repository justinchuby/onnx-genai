...
`onnx-runtime-ep-cuda` (the CUDA execution provider, `docs/architecture/ORT2.md` §15) only
...
## `onnx-runtime-ep-cpu` — publish considerations
...
kernel (`docs/architecture/ORT2.md` §25.2), so `cargo publish`/`cargo build` need no external
library, C++ toolchain, or network beyond crates.io.

The former oneDNN feature, its submodule, and its CMake/bindgen build path were
removed. The built-in `SimdX86` backend is the x86 fast path, while Generic
remains the portable fallback; both keep published builds offline.
