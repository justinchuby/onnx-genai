...
Attention now supports f32/f16 activations, cache tensors, and additive masks;
RoPE remains f32-only. A uniform f32 activation graph is the simplest
conservative export profile, while f16 Attention can avoid its cast/bandwidth
cost in otherwise mixed-precision graphs.
...
`Undefined`; present masks must be bool/f32/f16, present cache tensors must be paired
f32 or f16 values matching Q/K/V, and present nonpad lengths must be int64 with the opset/cache
