...
- **Benchmarking (§ new):** `onnx-genai-bench` cross-runtime harness (`compare.rs` / `scripts/compare_runtimes.sh`) measures TTFT + decode tok/s vs Ollama (llama.cpp) + LM Studio over the OpenAI API. Goal: beat llama.cpp + LM Studio (Metal). Runs recorded under `docs/benchmarks/`.
  ...
  - **🎯 Full Mobius op coverage (landed, 2026-07-13):** the MLX EP now translates **~70 op types** (up from 11) across all families — math/logical (Div/Relu/Tanh/Clip/Gelu/Exp/Log/Sqrt/Sin/Cos/Min/Max/Pow/Where/Equal/…), reductions (ReduceSum/Max/Mean/Min/SumSquare/CumSum/TopK), shape/data-movement (Gather/Concat/Reshape/Transpose/Unsqueeze/Slice/Split/Expand/Tile/Pad/…), norms (RMS/Layer/Simplified/Skip/Group/Lp/Batch), attention (GQA + Attention 23/24 + MultiHeadAttention), Conv/ConvTranspose/pooling, quant (MatMulNBits + GatherBlockQuantized symmetric & asymmetric), TensorScatter, CausalConvWithState. Built via the modular registry (one handler + claim + registration per op, zero ep.cc edits), delivered by parallel worktree agents. **251 pytest passing** (attention tests assert the node actually ran on MLX), ctest 3/3, E2E coherent, memory-safe (every mlx handle via `ctx.Keep`). Deliberately CPU (documented): Scan (nested subgraph), Linear/LightningAttention, PackedMHA, and various edge forms. Known engine follow-up: null-guard the GetCapability clustering pass for interior optional-input gaps.
