...
| 🔴 | oneDNN Windows feature | `crates/onnx-runtime-ep-cpu/build.rs:117-130`; `crates/onnx-runtime-ep-cpu/Cargo.toml:22-35` | The optional optimized CPU build links `stdc++` on every non-macOS target and defaults OpenMP to `gomp`. Those names are not valid for the normal MSVC oneDNN toolchain, so `--features onednn` is not Windows-ready. | Add target-specific C++/OpenMP linking: no explicit `stdc++` for MSVC, and select/allow the oneDNN-compatible Windows runtime. Add an MSVC CI lane for this non-default feature. |
