...
| `QMoE` | `com.microsoft` | ✅ | **NVRTC block-dequant expert GEMM** | Single-GPU resident Phase 1: exact ORT expert-major INT4/INT8 layouts, optional affine zero points and expert biases, top-k routing/normalization, Relu/Gelu/Silu/Identity and fused or separate-gate SwiGLU. f32/f16/bf16 activations use f32 accumulation; launch widths and saturation grids derive from the detected device capability. Weight paging/prefetch and expert-parallel sharding are deferred. |
