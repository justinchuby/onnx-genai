...
- Median paired steady-state throughput ratio must be at least `0.90`.
...
The ignored conformance test covers decoder + sampler + termination and parameterized min-p +
sampler + termination. Min-p remains fixed-shape and receives its threshold from the request; it
does not require artifact regeneration.
