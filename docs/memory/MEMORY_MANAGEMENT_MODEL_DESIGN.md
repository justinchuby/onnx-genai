...
|---|---|---|
| Virtual reservation | Request `max(16 × total VRAM, 1 TiB)`, then halve failed requests down to a 64 GiB minimum | Address space only; it does not reserve device memory. The accepted reservation is the arena ceiling. On the audited RTX 4060, the 1 TiB request fell back to a 512 GiB virtual reservation with **0 bytes physically committed** at provider construction. Do not report reserved VA as VRAM use. The standalone/plugin and managed-ORT allocators use the same policy. |
