...
> [DESIGN.md](../architecture/DESIGN.md) §26.11 & §43.2,
> [MOE_SUPPORT.md](../quantization/MOE_SUPPORT.md) §7,
> [MOE_EXPERT_PARALLELISM.md](../quantization/MOE_EXPERT_PARALLELISM.md) §8, and
> [DISTRIBUTED_RUNTIME.md](../distributed/DISTRIBUTED_RUNTIME.md) §3, §5, §12.
...
| Memory mechanism primitives | #1186 Phase 1 | **implemented** | dependency-free `crates/onnx-runtime-memory-api`; existing governor paths re-export the moved types |
| Allocator contract | §1.2, §1.3, §1.5 | **implemented on all three backends** | `DeviceAllocator` remains in `onnx-runtime-memory-governor/src/allocator.rs`; CPU EP, ONNX Runtime and CUDA EP each implement it |
...
Phase 1 of #1186 ships a dependency boundary without changing this contract.
`onnx-runtime-memory-api` now owns `Tier`, `DeviceKey`,
`AllocationCommitRange`, `MappedAllocation`, `SharedDevicePrefix`, and
`SharedPrefixCommitInfo`; the former governor paths re-export those exact types.
`DeviceAllocator` and `HostAllocator` remain in the governor crate because the
current trait signatures use governor-owned capacity tokens and `MemoryError`.
Roles, errors, authority and holder identities, ledgers, grants, leases,
pressure responders, shareability analysis, and policy also remain governance
concerns. This extraction does not split allocator capabilities or change eager
allocation, VMM, shared-prefix, mapped-refund, synchronization, selection, or
...
> See [DESIGN.md §26.11](../architecture/DESIGN.md) for the full governor design including
> config surfaces (YAML + Rust API + Python), error experience
> (`ResourceError` with what/why/how), and implementation status.

> See [DESIGN.md §43.2](../architecture/DESIGN.md) for the declaration that expert weights
...
> See [DISTRIBUTED_RUNTIME.md §3.1](../distributed/DISTRIBUTED_RUNTIME.md) for the canonical
...
[DISTRIBUTED_RUNTIME.md §4](../distributed/DISTRIBUTED_RUNTIME.md#4-communicator-backends).
...
> [MOE_EXPERT_PARALLELISM.md §8](../quantization/MOE_EXPERT_PARALLELISM.md) is superseded by
...
[DISTRIBUTED_RUNTIME.md §3.3](../distributed/DISTRIBUTED_RUNTIME.md#33-buffer-location-awareness).
...
Full design in [DISTRIBUTED_RUNTIME.md §5](../distributed/DISTRIBUTED_RUNTIME.md).

### 8.1 Format Negotiation at Boundaries

> See [DISTRIBUTED_RUNTIME.md §5.2](../distributed/DISTRIBUTED_RUNTIME.md) for the canonical
...
7. **Quantized communication.** Send FP8/INT8 and up-cast at receiver to halve
   ...
   **Decision:** Phase 1 full precision. Phase 3+ add optional quantized wire
   ...
   [DISTRIBUTED_RUNTIME.md §3.1](../distributed/DISTRIBUTED_RUNTIME.md#31-core-trait).
...
- [DESIGN.md §26.11](../architecture/DESIGN.md) — Resource Governor: canonical design (stays in place)
- [DESIGN.md §43.2](../architecture/DESIGN.md) — MoE Expert Weights: "not KV cache" declaration
- [WEIGHT_OFFLOAD.md](WEIGHT_OFFLOAD.md) — Three-tier weight residency (redirects here for §4)
- [MOE_SUPPORT.md](../quantization/MOE_SUPPORT.md) — First-class MoE support (redirects here for §7)
- [MOE_EXPERT_PARALLELISM.md](../quantization/MOE_EXPERT_PARALLELISM.md) — Session-per-GPU MoE architecture (DispatchTransport deprecated)
- [DISTRIBUTED_RUNTIME.md](../distributed/DISTRIBUTED_RUNTIME.md) — Communicator abstraction & multi-device inference
- [SCHEDULING.md](../genai/SCHEDULING.md) — Adaptive scheduling, EP negotiation protocol
