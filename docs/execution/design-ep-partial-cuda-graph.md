...
**Related:** [`RULES.md` §2 / §2.1](../../RULES.md), [`docs/execution/CUDA_GRAPH_CAPTURE.md`](CUDA_GRAPH_CAPTURE.md),
[`docs/execution/EP_CLAIM_DIAGNOSTICS.md`](EP_CLAIM_DIAGNOSTICS.md), [`docs/execution/NATIVE_CUDA_DECODE.md`](NATIVE_CUDA_DECODE.md),
[`docs/architecture/ORT2.md` §15.1 / §55.6](../architecture/ORT2.md)
...
(`docs/execution/CUDA_GRAPH_CAPTURE.md`). Capturing it collapses hundreds of per-step kernel
...
3. **Capture-safe kernel only.** The warmed kernel's [`CaptureSupport`] is
   ...
   (`docs/execution/CUDA_GRAPH_CAPTURE.md`). This is the kernel's *own* declared contract,
...
- **Interior of a capture region** references only fixed device addresses. A captured
  ...
  installed executable (`docs/execution/CUDA_GRAPH_CAPTURE.md`: *"destroyed before its
...
Prior lesson (`crates/onnx-genai-ort/src/cuda_rt.rs`, `docs/execution/CUDA_GRAPH_CAPTURE.md`):
...
4. **Capture master switch (already exists).** `ONNX_GENAI_CUDA_GRAPH=1` /
   `NativeDecodeCudaOptions::graph_capture` (`docs/execution/CUDA_GRAPH_CAPTURE.md`). Partial
...
3. **Invalidation.** Reset, rewind, prefill/multi-token shape changes, binding
   ...
   (`docs/execution/CUDA_GRAPH_CAPTURE.md`). A later step re-warms and re-captures a fresh
...
- The partition/policy types (`CaptureRegion`, seam kinds, path-kind enum) are pure
  ...
  see [`docs/architecture/CROSS_PLATFORM.md`](../architecture/CROSS_PLATFORM.md)), the CUDA EP is not compiled and
...
- **No SM assumption.** Capture eligibility is a function of kernel contracts and graph
  ...
  (consistent with `docs/architecture/CROSS_PLATFORM.md` PTX/arch handling).
...
buildable/reviewable (RULES §9) and does not touch `docs/status/PROGRESS.md` here.
