...
| `Einsum` | `` | ✅ | **canonical semantic/index plan + cuBLASLt / generic NVRTC / zero-copy views** | Exhaustive native CUDA execution for every legal homogeneous numeric expression: Einsum-12 f16/f32/f64 and signed/unsigned 8/16/32/64-bit integers, plus BF16 only under Einsum-28. The capture-safe GenericNative kernel handles arbitrary operand count, strides, diagonals, broadcast products, named/reduced ellipses, reductions, scalars, zero extents, and arbitrary output order without host materialization or exponential intermediates. Bounded exact-DP/greedy trees use reusable device temporaries and eligible f32 binary steps use cuBLASLt; direct contiguous f16/f32 GEMM/BMM and zero-copy view paths remain fast paths. |
