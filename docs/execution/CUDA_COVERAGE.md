...
| `Einsum` | `` | ✅ | **canonical `EinsumPlan` + cuBLASLt / zero-copy views** | Opset 12. Capture-safe f32/f16 contiguous binary GEMM/BMM contractions, operand transpose descriptors, equal or whole-operand stride-zero batch broadcasting, dot/rank promotion, zero dimensions, and zero-copy permutation/diagonal views. BFloat16 is deliberately declined because it is not in the ONNX opset-12 Einsum type constraint, despite MatMul supporting it. Output permutations, partial multi-axis batch broadcasting, uncoupled reductions/elementwise products, and general N-way contractions also decline with actionable reasons. |
