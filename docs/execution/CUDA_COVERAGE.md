...
| `DsaIndexSelect` | `pkg.nxrt` | ✅ | **NVRTC index-selection kernel** | Device-resident query-dependent sparse index selection for the GLM-5.2 DSA / `IndexShare` indexer. Reproduces the frozen CPU reference (`onnx-runtime-ep-cpu`'s `dsa_index_select.rs`) on the device: head-weighted ReLU'd scaled dot-product scoring, additive f32 causal bias, a `bias > -1e30` allow mask, then the top `min(#allowed, top_k)` positions by `(score desc, index asc)`, emitted ascending and right-padded with the `-1` sentinel. Final NaNs are canonicalized to positive quiet NaN on both backends, so selection is bit-parity with the CPU oracle. |
