...
| P1 | ⚠️ **The shape-inference registry now contains 181 operators / 219 versioned entries.** A later Issue #75 batch adds 16 more standard tensor operators: `ConvTranspose` (deconvolution formula, `output_shape`/`SAME_*` handling), `GridSample`, `Einsum` (equation-driven, with ellipsis broadcasting), the `Celu`/`Shrink`/`Mish`/`HardSwish` activations, `LRN`, `MeanVarianceNormalization`, `ReverseSequence`, `RandomNormal`/`RandomUniform` and their `*Like` variants, `Bernoulli`, and `Multinomial`. Symbolic/unknown extents degrade to stable fresh symbols rather than fabricated constants, and every rule is `since_version`-gated. | Kernel-backed tensor operators are substantially covered; full ONNX shape-inference parity still depends on container-valued sequence/optional types (the IR carries only tensor `dtype`+`shape` on graph values) and control-flow rules beyond `If`. |
...
| `TensorShapeProto` | `dim`(1) | ✅ | ⚠️ rank is represented; the shape registry covers 181 operators / 219 versioned entries, while the incomplete container/control-flow catalog limits full shape coverage | ✅ tensor/sparse signatures | ✅ | ✅ |
