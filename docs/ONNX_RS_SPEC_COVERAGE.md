...
| P1 | ⚠️ **The shape-inference registry now contains 164 operators / 203 versioned entries.** Issue #75 closes kernel-without-inference gaps for the same 16 standard operators, including symbolic broadcasting, data-dependent `Unique` extents, both `Dropout` outputs, predicate boolean outputs, and opset-gated registrations. Dynamic extents receive stable symbols rather than fabricated constants. | Kernel-backed tensor operators are substantially covered; full ONNX shape-inference parity still depends on container-valued sequence/optional types and control-flow rules beyond `If`. |
...
| `TensorShapeProto` | `dim`(1) | ✅ | ⚠️ rank is represented; the shape registry covers 164 operators / 203 versioned entries, while the incomplete container/control-flow catalog limits full shape coverage | ✅ tensor/sparse signatures | ✅ | ✅ |
