...
| `DeviceBuffer` struct itself cannot cross FFI | Contains `NonNull<c_void>`, `usize`, `usize`, `BufferOwner` (private enum) | **ABI-safety** | ✅ Resolved for CPU. CUDA path (device pointers) exercised on an H200 in #832 — see `docs/CUDA_EP_STATUS.md`. |
