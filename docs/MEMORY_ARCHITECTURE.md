...
| `--vram-limit` | — | **fixed 2026-08-06** | Was: bounds only the KV budget derivation, so a 2 GB ceiling on a model needing ~15.5 GB still generated successfully, warning and proceeding (#712). Now enforced at load and it auto-enables weight offload, deriving the weight budget as `limit − device state`. At 6 GB on qwen2.5-14b: `offload_device_budget_bytes=4389387264` = 6e9 − 1,610,612,736 of KV, loads and generates. At 1 GB it fails **at load**, naming every term. Caveat carried, not closed: `activations_bytes=unknown` and `runtime_overhead_bytes=unknown` are not subtracted, so a limit can still be exceeded in practice — the engine says so aloud rather than reporting zero (#514). PR #717 |
