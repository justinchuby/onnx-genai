#!/usr/bin/env python3
"""Generate a tiny GroupQueryAttention ONNX fixture that exercises the
shared-buffer (fixed-capacity in-place KV) decode path.

Default command from the onnx-genai repo root:

    PYTHONPATH=/path/to/mobius/src \
      python tests/fixtures/tiny-llm-sharedbuffer/generate_tiny_llm_sharedbuffer.py

Unlike the sibling ``tiny-llm`` fixture (which builds with the ``default``
execution provider and keeps standard growing-KV ``Attention`` ops), this
fixture builds with the ``cpu`` execution provider. The Mobius ``cpu`` EP
declares ``gqa_dtypes={FLOAT}``, so the fused attention is lowered to a
float32 ``com.microsoft.GroupQueryAttention`` node. That op keeps its past
and present KV tensors in a single fixed-capacity buffer, which is exactly
the graph shape ``DecodeSession``'s ``SharedBuffer`` decode mode drives.
...
def main() -> None:
    ...
    config = _make_config()
    pkg = build_from_module(GPT2CausalLMModel(config), config, execution_provider="cpu")
    ...
    manifest = {
        "generator": "tests/fixtures/tiny-llm-sharedbuffer/generate_tiny_llm_sharedbuffer.py",
        "execution_provider": "cpu",
        "attention_op": "com.microsoft.GroupQueryAttention",
        "seed": SEED,
