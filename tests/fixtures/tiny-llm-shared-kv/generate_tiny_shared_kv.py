#!/usr/bin/env python3
"""Generate a tiny shared-KV (past_present_share_buffer) decoder ONNX fixture.

This mirrors ``tests/fixtures/tiny-llm/generate_tiny_llm.py`` but targets the
CUDA execution provider, which fuses GroupQueryAttention and enables the
``past_present_share_buffer`` KV layout. The result exercises the
``DecodeSession`` shared-buffer decode path (``DecodeKvMode::SharedBuffer``).

The fixture is used by the CUDA-gated integration test that verifies the
uncaptured persistent decode path never claims a CUDA-graph id.

Default command from the onnx-genai repo root:

    PYTHONPATH=C:\\Users\\justinchu\\dev\\mobius\\src \
      python tests/fixtures/tiny-llm-shared-kv/generate_tiny_shared_kv.py

The model is intentionally tiny and randomly initialized: vocab=32, hidden=16,
one decoder layer, two attention heads, max sequence length 16. It is only for
smoke tests, not quality evaluation.
"""
...
def main() -> None:
    ...
    pkg = build_from_module(
        GPT2CausalLMModel(config), config, execution_provider=EXECUTION_PROVIDER
    )
    ...
    manifest = {
        "generator": "tests/fixtures/tiny-llm-shared-kv/generate_tiny_shared_kv.py",
