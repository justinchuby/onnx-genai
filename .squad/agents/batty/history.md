...
## 2026-07-14T14:50:00Z — batty-19: GELU Erf-decomposition fusion (8e8d806)

Implemented the GELU fusion pass completing Phase-2 OpFusion on bert_toy. New `RewriteKind::Gelu` DAG matcher in `optimizer/src/fusion.rs` matching `Mul(X,0.5)×(1+Erf(Div(X,√2)))` with strict constant guards (±1e-6) and diamond closure (same `ValueId`). New CPU kernel `ep-cpu/src/kernels/gelu.rs` reusing the `erf` helper (made `pub(crate)`); registered `("Gelu","com.microsoft",1)`. Fires 6× on bert_toy; 0 Erf surviving; all other fusion counts unchanged. Parity: fused-vs-unfused 3.353e-8, vs-reference 1.192e-7, drift 1.416e-7; tolerances NOT loosened. Commit `8e8d806`. **Reviewer: roy-21 🟢.** **Phase-2 OpFusion set COMPLETE: LayerNorm / FusedMatMulBias / FusedGemm / FusedAttention / Gelu.**
