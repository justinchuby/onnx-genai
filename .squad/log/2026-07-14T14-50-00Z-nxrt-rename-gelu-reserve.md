...
Renamed 17 `ort2_*` → `nxrt_*` `extern "C"` symbols in capi (lib.rs + tests/capi.rs). Zero `ort2_` remaining in `crates/`. Preserved `docs/architecture/ORT2.md` citations and `ort2-session`/`ort2-ep-api` label strings (intentional, out of scope). No alias shims. Merged as `43292ee`.

### 3. Crate-name reservation prep + cycle fix (deckard-23 / leon-17, two commits)
- `8988abd`: All 8 `onnx-runtime-*` crates → `0.1.0-dev.0`; exact `=0.1.0-dev.0` workspace pins; `docs/architecture/CRATE_RESERVATION.md` runbook.
