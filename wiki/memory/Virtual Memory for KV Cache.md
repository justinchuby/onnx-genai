...
stateDiagram-v2
    ...
    Reserved --> Committed: commit<br/>(governor 批准 → cuMemCreate + cuMemMap + cuMemSetAccess)
    Committed --> Committed: KV 增长<br/>(在同段 VA 后面追加映射,指针不变)
    Committed --> Shared: 前缀共享<br/>(同一 handle 以 PROT_READ 多映射,只计费一次)
    Shared --> Committed: 所有 sharer 退出<br/>(refcount → 0)
    Committed --> Reserved: release()<br/>(unmap + 还句柄,<b>VA 保留可复用</b>)
