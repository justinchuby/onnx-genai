...
export PYTHONPATH="$MOBIUS_PYTHONPATH"

"$MOBIUS_PYTHON" - "$MODEL_ID" "$LOCAL_DIR" <<'PY'
...
"$MOBIUS_PYTHON" -m mobius build --config "$LOCAL_DIR" --runtime ort-genai "$OUT_DIR"

cat <<EOF

Built $MODEL_ID at $OUT_DIR.
Smoke test:
  cargo run -p onnx-genai-cli --bin onnx-genai -- generate "$OUT_DIR" \\
    --max-new-tokens 30 --prompt "Once upon a time"
EOF
